...
const UPDATE_CAMERA_TRAP_EVENT_MUTATION = gql`
  mutation UpdateCameraTrapEventMutation($id: String!, $input: UpdateCameraTrapEventInput!) {
...
export const Success = ({ cameraTrapEvent }: CellSuccessProps<EditCameraTrapEventById>) => {
  const [updateCameraTrapEvent, { loading, error }] = useMutation(UPDATE_CAMERA_TRAP_EVENT_MUTATION, {
    onCompleted: () => {
  ...
  return (
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">Edit CameraTrapEvent {cameraTrapEvent.id}</h2>
      </header>
      <div className="rw-segment-main">
        <CameraTrapEventForm cameraTrapEvent={cameraTrapEvent} onSave={onSave} error={error} loading={loading} />
      </div>
    </div>
  )
}
